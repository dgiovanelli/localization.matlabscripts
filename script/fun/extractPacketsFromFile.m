function packets = extractPacketsFromFile(fileID,options)
...
for packetNo = 1:size(year,1)
    if strcmp(data_type(packetNo),options.ADV_TYPE_IDENTIFIER) || strcmp(data_type(packetNo),options.GATT_TYPE_IDENTIFIER) || strcmp(data_type(packetNo),options.TAG_TYPE_IDENTIFIER)
